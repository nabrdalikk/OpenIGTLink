...
int main(int argc, char* argv[])
{
  ...
  pointToSend->SetName("POINT_0");
  pointToSend->SetGroupName("GROUP_0");
  pointToSend->SetRGBA(0xFF, 0x00, 0x00, 0xFF);
  pointToSend->SetPosition(10.0, 20.0, 30.0);
  pointToSend->SetRadius(15.0);
  pointToSend->SetOwner("IMAGE_0");
  ...
  pointMsg->AddPointElement(pointToSend);
  ...
  while (1)
  {
	for (int i = 0; i < 100; i ++)
	{
		...
		igtl::TimeStamp::Pointer tstamp;
		tstamp = igtl::TimeStamp::New();
		...
		headerMsg->GetTimeStamp(tstamp);
		tstamp->GetTimeStamp(&sec, &nanosec);
