...
int main(int argc, char* argv[])
{
	...
	while (1)
	{
		std::cerr << "Receiving POINT data type from server." << std::endl;
		...
		pointMsg2->SetMessageHeader(headerMsg);
		pointMsg2->AllocatePack();

		std::cerr << "HeaderMsg created" << std::endl;
			// Initialize receive buffer
		headerMsg->InitPack();
		std::cerr << "HeaderMsg initiated" << std::endl;

			// Receive generic header from the socket
			int r = socket->Receive(headerMsg->GetPackPointer(), headerMsg->GetPackSize());
			std::cerr << "HeaderMsg received" << std::endl;
			...
			headerMsg->Unpack();
			std::cerr << "HeaderMsg unpacked" << std::endl;

			
			
			socket->Receive(pointMsg2->GetPackBodyPointer(), pointMsg2->GetPackBodySize());
			std::cerr << "Received!" << std::endl;

			int c = pointMsg2->Unpack(1);

			if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
			{

				int nElements = pointMsg2->GetNumberOfPointElement();
				for (int i = 0; i < nElements; i++)
				{
					igtl::PointElement::Pointer pointElement;
					pointMsg2->GetPointElement(i, pointElement);
